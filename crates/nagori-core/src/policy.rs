...
impl SensitivityClassifier {
    ...
    /// entry and combines the result with the stored sensitivity, taking
    /// whichever is more restrictive.
    ///
    /// Every payload in both sets is scanned with the built-in detectors and
    /// the user rules; the sets differ only in how the entry-size ceiling
    /// treats them, mirroring the split `classify` makes at capture:
    ///
    /// - `sized` is charged to the ceiling, deduped by value, exactly as
    ///   `classify` sums the plain body, the rich-text markup, and the
    ///   `InlineText` representations.
    /// - `scan_only` is not. It covers what `classify` scans without
    ///   summing (the joined paths of a `FilePaths` representation) plus the
    ///   projections *computed from* `sized` — the normalized search text,
    ///   which is what would actually be embedded. Normalization folds case
    ///   and width and can hide a match the raw form shows, so the
    ///   projection must be scanned; but it adds no bytes to the entry, so
    ///   charging it would double-count the body and re-classify an entry
    ///   captured well under the limit as `Oversized`.
    ///
    /// Getting that split wrong is not a conservative error here: an
    /// `Oversized` verdict refuses the entry *and* tombstones it, so a
    /// spurious one drops it out of the index until its content changes.
    #[must_use]
    pub fn assess_semantic_texts(
        ...
        let mut counted: Vec<&str> = sized.to_vec();
        counted.sort_unstable();
        counted.dedup();

        let mut reasons = Vec::new();
        let sized_len: usize = counted.iter().map(|s| s.len()).sum();
        if sized_len > self.settings.max_entry_size_bytes {
            reasons.push(SensitivityReason::Oversized);
        }
        ...
        let mut scanned: Vec<&str> = scan_only.iter().chain(sized.iter()).copied().collect();
