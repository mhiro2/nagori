...
pub struct PendingEmbedding {
    ...
    pub text: String,
    /// The persisted text payloads that count toward the entry-size ceiling:
    /// the raw plain-text body, the rich-text markup, and the inline-text
    /// rows of `entry_representations` — exactly the set capture-time
    /// `classify` sums. The indexer re-assesses them against the current
    /// policy alongside `text`, because a rule can match a markup or
    /// alternative payload that never reached the plain projection.
    pub sized_texts: Vec<String>,
    /// Persisted text the current policy must be re-scanned against but
    /// which capture-time `classify` did *not* charge to the size ceiling:
    /// the newline-joined paths of `text/uri-list` (`FilePaths`) rows.
    /// Kept apart from [`Self::sized_texts`] so the indexer's re-assessment
    /// reaches the same `Oversized` verdict the capture did.
    pub scan_only_texts: Vec<String>,
...
///
/// Returns `(sized_texts, scan_only_texts, content_unparseable)`, split the
/// way capture-time `classify` splits them: it sums the plain body, the
/// markup, and the `InlineText` representations against the size ceiling but
/// only *scans* `FilePaths` rows. Charging the joined paths too would let a
/// file list that classified fine at capture come back `Oversized` here.
///
/// The flag is set (and surfaced to the indexer so it can fail closed) when
/// `content_json` no longer deserializes: a body that cannot be re-checked
/// against the current policy must not be embedded on the normalized
/// projection alone.
fn recover_raw_texts(
    ...
    let mut sized_texts = Vec::new();
    let mut scan_only_texts = Vec::new();
    ...
    for rep in reps {
        ...
        // `text/uri-list` is how a `FilePaths` representation persists (see
        // `decode_representation_payload`), and its `text_content` is a
        // JSON-encoded path list. The capture-time classifier scanned those
        // paths joined with newlines, so decode back to the same shape — a
        // JSON blob would defeat anchored or quote-sensitive rules that
        // matched at capture. `decode_file_paths` cannot fail (a non-JSON
        // legacy row falls back to its newline form).
        if mime.eq_ignore_ascii_case("text/uri-list") {
            scan_only_texts.push(nagori_core::decode_file_paths(&text).join("\n"));
        } else {
            sized_texts.push(text);
        }
    }
    Ok((sized_texts, scan_only_texts, content_unparseable))
}
    ...
    pub async fn semantic_pending(&self, limit: usize) -> Result<Vec<PendingEmbedding>> {
        ...
        self.run_blocking(move |store| {
            ...
            for (id, content_hash, text, sensitivity, source, content_json) in base {
                ...
                let (sized_texts, scan_only_texts, content_unparseable) =
...
mod tests {
    ...
    async fn pending_recovers_representation_texts_for_reassessment() {
        ...
        assert!(
            pending[0].sized_texts.iter().any(|t| t == "plain body"),
            "raw body must be recovered: {:?}",
            pending[0].sized_texts
        );
        assert!(
            pending[0]
                .sized_texts
                .iter()
                .any(|t| t.contains("ACME-1234")),
            "alternative-representation text must be recovered: {:?}",
            pending[0].sized_texts
        );
        assert!(
            pending[0].scan_only_texts.is_empty(),
            "inline-text representations are size-bearing, not scan-only: {:?}",
            pending[0].scan_only_texts
        );
    }
    ...
    async fn pending_decodes_file_path_representations_to_the_capture_shape() {
        ...
        // Capture-time `classify` scans a `FilePaths` representation without
        // charging it to the size ceiling; the re-assessment must match, or a
        // file list that classified fine at capture would come back
        // `Oversized` and be tombstoned out of the index.
        assert!(
            !pending[0].sized_texts.iter().any(|t| t.contains(".txt")),
            "file paths must not be charged to the size ceiling: {:?}",
            pending[0].sized_texts
        );
    }
