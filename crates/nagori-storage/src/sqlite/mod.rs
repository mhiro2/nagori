...
mod tests {
    ...
    async fn settings_revision_bumps_and_cas_rejects_stale_writes() {
        let store = SqliteStore::open_memory().unwrap();
        // Fresh store: no settings row yet, so the revision baseline is 0.
        assert_eq!(store.settings_revision().await.unwrap(), 0);
        ...
        let rev = store
            ...
            .unwrap();
        assert_eq!(rev, 1);
        assert_eq!(store.settings_revision().await.unwrap(), 1);
        ...
        assert_eq!(store.settings_revision().await.unwrap(), 3);
    }
