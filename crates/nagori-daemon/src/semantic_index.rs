...
/// payload that never reached the plain projection. The size ceiling is
/// charged to the same payloads `classify` sums at capture and not to what it
/// only scans, so an entry that fit under it at capture cannot re-classify as
/// `Oversized` here. `Private` bodies are
...
fn partition_by_current_policy<'p>(
    ...
    for entry in pending {
        ...
        // The normalized text is a projection of the stored payloads rather
        // than a payload of its own, so it joins the file-path rows that
        // capture-time `classify` scans without summing: scanned here, but
        // not charged against the entry-size ceiling.
        let mut scan_only: Vec<&str> = Vec::with_capacity(1 + entry.scan_only_texts.len());
        scan_only.push(entry.text.as_str());
        scan_only.extend(entry.scan_only_texts.iter().map(String::as_str));
        let sized: Vec<&str> = entry.sized_texts.iter().map(String::as_str).collect();
        let reassessed =
            classifier.assess_semantic_texts(&scan_only, &sized, entry.source.as_ref());
