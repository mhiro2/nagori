...
**Plans.** `SearchPlan::resolve(mode, normalized)` chooses between
...
**Bounded candidate projection.** Search candidates never carry the stored
`normalized_text`. SQLite computes a typed `TextMatch` (`Exact`, `Prefix`,
...
**Ranker.** `DefaultRanker` combines the provider's typed text-match signal
with the index scores and metadata signals:
...
batch before its vectors persist. The entry-size ceiling is charged to the
same payloads `classify` sums at capture — the plain body, the markup, and
the inline-text representations — and not to what it merely scans (the joined
paths of a file-list representation) or to the normalized projection derived
from them, so an entry that fit under the limit at capture cannot
re-classify as `Oversized` here and be tombstoned out of the index. The
atomicity anchor is the
settings write itself: a save whose policy fingerprint differs erases the
vectors, tombstones, and metadata row *in the same transaction*
(`invalidate_semantic_index_on_policy_change`), and the indexer's guarded
upsert / exclusion writes re-check
...
- **Tauri command ACL** — `build.rs` declares every `generate_handler!`
  ...
  entirely; the frontend ships no `commands.ts` binding for them either, so
  the JS invoke surface matches the granted set and a runtime deny cannot be
  hit by accident. The autogenerated permission TOMLs live under
  `permissions/autogenerated/`. The command list in `build.rs` must stay in
  lockstep with `generate_handler!`: a command registered but absent from
  the manifest has no permission and is rejected for every window. This
  invariant is enforced mechanically — `tests/command_surface.rs` asserts
  that the manifest and `generate_handler!` list the same commands, that
  every granted permission maps to a real command, and that the two webviews
  share only the audited read-only commands — so a future command addition
  cannot silently widen or break the ACL. Every command's effect classification
  (read-only / write / destructive / external side effect / window) and its
  window exposure are tabulated in [`docs/command-surface.md`](docs/command-surface.md).
