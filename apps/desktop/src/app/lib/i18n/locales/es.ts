  ...
  settings: {
    ...
    ai: {
      ...
      semanticIndexProgress: ({ percent, indexed, total, pending }) =>
        `${percent}% (${indexed.toLocaleString('es')} / ${total.toLocaleString('es')}, ${pending.toLocaleString('es')} pendientes)`,
    ...
    hotkeys: {
      ...
      paletteHelp:
        'Atajos que se usan dentro de la paleta. Las acciones que no has personalizado usan sus teclas por defecto.',
      secondaryHeading: 'Atajos globales secundarios',
      secondaryHelp:
        'Atajos globales opcionales que funcionan sin abrir la paleta. Configura solo las acciones que necesites; las acciones sin configurar están desactivadas.',
      placeholder: 'Definir atajo',
      recordingHint: 'Pulsa el atajo…',
      recordingCancelHint: 'Esc para cancelar',
      clearAriaLabel: 'Borrar atajo',
      defaultMarker: 'Por defecto',
      disabledMarker: 'Desactivado',
      notSet: 'Sin definir',
      reset: 'Restablecer',
      fieldAriaLabel: 'Atajo para {action}',
      restoreDefault: 'Restablecer el valor por defecto de {action}',
      removeShortcut: 'Quitar el atajo de {action}',
      disableShortcut: 'Desactivar {action}',
      paletteActions: {
        pin: 'Alternar fijado',
