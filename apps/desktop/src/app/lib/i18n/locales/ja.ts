...
export const ja: Messages = {
  ...
  status: {
    ...
    pasteDiagnostics: {
      ...
      hint: {
        ...
        clipboardChanged:
          '自動ペーストを中止: コピー後にクリップボードが変わったため、何も貼り付けていません。もう一度コピーしてください。',
  ...
  settings: {
    ...
    privacy: {
      ...
      permanentDeleteOnDelete: '削除した項目をすぐにディスクから消去する',
      permanentDeleteOnDeleteHelp:
        '有効にすると、項目を削除した時点でディスクからも消去します。無効のときは、削除した項目は一覧からすぐ消えますが、ディスクからの消去は少しあとの定期処理で行われます。シークレットと判定された項目は、設定に関わらず常にすぐ消去されます。',
