...
export const fr: Messages = {
  palette: {
    ...
    fileList: {
      ...
      locations: (count) =>
        count === 1 ? '1 emplacement' : `${count.toLocaleString('fr')} emplacements`,
      rowAria: ({ total, names, location }) => {
        const head = total === 1 ? names : `${total.toLocaleString('fr')} fichiers : ${names}`;
        return location ? `${head}, dans ${location}` : head;
      },
    },
  },
  rankReason: {
  ...
  actionMenu: {
    ...
    notApplicable: {
      ...
      url: 'Cette action ne s’applique pas aux URL.',
    },
    aiRemediation: {
  ...
  pastePicker: {
    ...
    keepOriginal: 'Conserver le format d’origine',
    categories: {
      files: 'Fichiers',
      image: 'Image',
  ...
  settings: {
    ...
    hotkeys: {
      ...
      paletteHelp:
        'Raccourcis utilisés au sein de la palette. Les actions non personnalisées utilisent leurs touches par défaut.',
      secondaryHeading: 'Raccourcis globaux secondaires',
      secondaryHelp:
        'Raccourcis globaux optionnels qui fonctionnent sans ouvrir la palette. Ne définissez que les actions nécessaires ; les actions non définies sont désactivées.',
      placeholder: 'Définir le raccourci',
      recordingHint: 'Appuyez sur la combinaison…',
      recordingCancelHint: 'Échap pour annuler',
      clearAriaLabel: 'Effacer le raccourci',
      defaultMarker: 'Par défaut',
      disabledMarker: 'Désactivé',
      notSet: 'Non défini',
      reset: 'Réinitialiser',
      fieldAriaLabel: 'Raccourci pour {action}',
      restoreDefault: 'Rétablir la valeur par défaut pour {action}',
      removeShortcut: 'Supprimer le raccourci pour {action}',
      disableShortcut: 'Désactiver {action}',
      paletteActions: {
        pin: 'Basculer l’épinglage',
        delete: 'Supprimer l’élément',
        'paste-as-plain': 'Coller au format…',
        'copy-without-paste': 'Copier dans le presse-papiers',
